<div class="section features-6" id="desajuara">
  <div class="container">
    <div class="row">
      <div class="col-md-8 mx-auto text-center">
        <span class="badge badge-warning badge-pill mb-3 font-weight-bold" style="font-size: 16px">Tentang Kami</span>
        <h3 class="display-3">Desajuara</h3>
        <p class="lead mb-5">Desajuara merupakan sebuah platform yang berupaya membantu desa dalam memaksimalkan perencanaan pembangunan dan pemberian layanan kepada masyarakat. Fitur unggulan desajuara adalah optimalisasi peta bidang yang membantu meminimalisir permasalahan lahan dan pengembangan potensi desa.</p>
      </div>
    </div>
    <div class="row align-items-center">
      <div class="col-lg-6">
        <div class="card border-0 mb-5" style="border-radius: 1rem">
          <div class="info info-horizontal info-hover-primary">
            <div class="card-body">
            <div class="description">
                <h5 class="title font-weight-bold" style="font-size: 1.5rem">Terintegrasi</h5>
                <p style="font-size: 1.1rem">Sekarang kamu dapat lebih mudah dan lebih cepat memberi pelayanan kepada masyarakat, dan anggota masyarakat bisa mengakses pelayanan kapan saja dimana saja.</p>
                {{-- <a href="#" class="text-info">Learn more</a> --}}
              </div>
            </div>
          </div>
        </div>
        <div class="card border-0 mb-5" style="border-radius: 1rem">
          <div class="info info-horizontal info-hover-primary ">
            <div class="card-body">
              <div class="description">
                <h5 class="title font-weight-bold" style="font-size: 1.5rem">Partisipatif</h5>
                <p style="font-size: 1.1rem">Kami membantu untuk memaksimalkan partisipasi masyarakat agar kamu berkembang semakin pesat.</p>
                {{-- <a href="#" class="text-info">Learn more</a> --}}
              </div>
            </div>
          </div>
        </div>
        <div class="card border-0 mb-5" style="border-radius: 1rem">
          <div class="info info-horizontal info-hover-primary ">
            <div class="card-body">
              <div class="description">
                <h5 class="title font-weight-bold" style="font-size: 1.5rem">Lihat Peta - Holistik</h5>
                <p style="font-size: 1.1rem">Pengin tau tentang kondisi, status, dan informasi bidang bidang tanah di desa? Sekarang kamu bisa melihatnya dalam genggaman dengan aman dan nyaman.</p>
                {{-- <a href="#" class="text-info">Learn more</a> --}}
              </div>
            </div>
          </div>
        </div>
        <div class="card border-0" style="border-radius: 1rem">
          <div class="info info-horizontal info-hover-primary ">
            <div class="card-body">
              <div class="description">
                <h5 class="title font-weight-bold" style="font-size: 1.5rem">Planner - Terencana</h5>
                <p style="font-size: 1.1rem">Bersama kami, ayo efisiensikan program dan kegiatan dengan perencanaan yang tertata agar menghasilkan kebermanfaatan yang lebih besar.</p>
                {{-- <a href="#" class="text-info">Learn more</a> --}}
              </div>
            </div>
          </div>
        </div>
      </div>
      <div class="col-lg-6 col-10 mx-md-auto">
        <img class="ml-lg-5" src="{{url('frontend/argon_theme/assets/img/ill/ill.png')}}" width="100%">
      </div>
    </div>
  </div>
</div>
